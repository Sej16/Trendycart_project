...
class ColorComponent extends Component {
	...
	handleTextImport() {
		...
		if (
			textImportData &&
			textImportData instanceof Array &&
			textImportData.length === 9 &&
			textImportData[0] &&
			textImportData[0].color
		) {
	...
	render() {
		...
		Object.keys(this.controlParams.colors).map((item, index) => {
			return currentPaletteData.push({
				color:
					undefined !==
						this.state.value[this.state.value.active][index] &&
					this.state.value[this.state.value.active][index].color
						? this.state.value[this.state.value.active][index].color
						: "",
			});
		});
